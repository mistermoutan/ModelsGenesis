...
class _ACSConv(nn.Module):
    ...
    def __init__(
        ...
        if self.transposed:
            ...
            )  # should not *_pair_same(kernel_size) be 3 elements for the filters?
            # no, this is where unsqueeze in functional comes in, just add a dimension has there is
            # convolution along 2D, however the basis is the same! the extra dimension just gets added later for each
            # axes to do a 3d conv
        else:
            self.weight = nn.Parameter(torch.Tensor(out_channels, in_channels // groups, *_pair_same(kernel_size)))
