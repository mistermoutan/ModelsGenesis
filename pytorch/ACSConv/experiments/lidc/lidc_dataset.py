    ...
    def __init__(self, crop_size, move, data_path, train=True, copy_channels=True):
        ...
        if train:
            self.names = (
                ...
                .dropna()
                .map(lambda x: os.path.join(self.data_path, "nodule", x))
                .values
            )
        else:
            self.names = (
...
class Transform:
    ...
    def __call__(self, voxel, seg):
        ...
        if self.copy_channels:
            return np.stack([voxel_ret, voxel_ret, voxel_ret], 0).astype(np.float32), np.expand_dims(seg_ret, 0).astype(np.float32)
        else:
            return np.expand_dims(voxel_ret, 0).astype(np.float32), np.expand_dims(seg_ret, 0).astype(np.float32)
...
class LIDCTwoClassDataset(Dataset):
    def __init__(self, crop_size, move, data_path, train=True, copy_channels=True):
        ...
        self.names = pd.merge(pd.Series(self.info.index.map(lambda x: x + ".npz"), name="index"), pd.Series(self.names, name="index"))[
            "index"
        ].unique()
