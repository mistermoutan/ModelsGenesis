...
class GeneratePairBuffer:
    ...
    def start_processes(self): 
        ...
        self.p_tr = multiprocessing.Process(target=self._tr_process)
        self.p_val = multiprocessing.Process(target=self._val_process)
        self.p_ts = multiprocessing.Process(target=self._ts_process)
        self.p_tr.start()
        self.p_val.start()
        self.p_ts.start()
    ...
    def _ts_process(self):
        ...
        while True:
            x, _ = self.dataset.get_test(batch_size=self.config.batch_size_ss, return_tensor=False)
